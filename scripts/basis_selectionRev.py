...
def calculate_flops(active_groups: np.ndarray, memory_depth_M: int, num_bands: int) -> float:
    ...
    imd_indices = {12, 13, 14, 15, 16, 17, 18, 19}

    active_groups = np.asarray(active_groups, dtype=int)
    if active_groups.size == 0:
        return 0.0

    num_linear = int(np.isin(active_groups, list(linear_indices)).sum())
    num_envelope = int(np.isin(active_groups, list(envelope_indices)).sum())
    num_imd = int(np.isin(active_groups, list(imd_indices)).sum())

    total_groups = (num_linear + num_envelope + num_imd) * num_bands
    R = total_groups * memory_depth_M

    nonlinear_costs = (
        num_envelope * num_bands * memory_depth_M * 2
        + num_imd * num_bands * memory_depth_M * 12
    )

    return float(8 * R - 2 + 3 + nonlinear_costs)
...
def block_omp_sweep(
    ...
    for k in range(1, max_groups + 1):
        ...
        flop_counts.append(calculate_flops(active_idx, memory_depth_M, num_bands))
...
def plot_pareto(
    ...
    """
    Plot BOMP Pareto frontiers (both Active Groups vs NMSE and FLOPs vs NMSE).
    
    Args:
        band_results_dict: Dictionary mapping band name (y1, y2, y3) to SweepResult
        output_dir: Directory to save both plots
    """
...
def main() -> None:
    ...
    # Generate both Pareto plots
    plot_pareto(bomp_results_per_band, args.output_dir)
    print(f"✓ Pareto plots saved to: {args.output_dir}")
